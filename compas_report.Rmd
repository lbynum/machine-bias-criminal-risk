...
Now that we have gone through ProPublica’s analysis, we will create our own logistic regression model to predict recidivism using ProPublica’s dataset. Given that COMPAS scores measure the risk of recidivating, we can create our own version of a COMPAS score by simply predicting the probability of recidivating. To keep the scope of the project manageable, our focus here will be minimizing racial bias. For our model, we chose to include age, sex, juvenile infraction/misdemeanor/felony counts, prior offenses count, and current charge degree (felony or misdemeanor). Although the dataset contains additional information about each individual, we did not include those in our model because it is unique to an individual (name, id, etc.) or related to a follow-up crime (and we cannot use follow-up crime information in predicting whether or not an individual will recidivate). Additionally, we focused primarily on racial bias in our analysis, but we could imagine removing age and/or sex from our model if we wanted to investigate those biases as well.
...
The COMPAS Model has a predictive accuracy of roughly 62\%. Using 10-fold cross validation, we were able to gather predictions for each individual in our dataset and achieve an accuracy of approximately 68\%. Although the difference is not drastic, it is still a significant improvement over the COMPAS scoring model’s performance. Additionally, our algorithm only uses a handful of features for each individual, while the COMPAS algorithm leverages data from a far more involved, detailed assessment.
...
```{r}
new_df <- our_scores %>%
  select(age, c_charge_degree, race, age_cat, new_score_text, sex, 
         priors_count, days_b_screening_arrest, new_score, is_recid, 
...
As mentioned before, to narrow the scope of this report, we focus on *racial bias* against black people as compared to white people, and we implement sampling to combat this type of bias only. If our example words for racial bias, we could theoretically extend it to each of the protected attributes in the data.
...
We create these partitions for our data.
...
The process for sampling to get a non-discrimatory distribution is as follows.
...
3. Train the model on the sampled (now bias-free) data
...
post_sample_df <- post_sample_scores %>%
  select(age, c_charge_degree, race, age_cat, new_score_text, sex, priors_count, 
         days_b_screening_arrest, new_score, is_recid, two_year_recid, 
         c_jail_in, c_jail_out) %>% 
  filter(days_b_screening_arrest <= 30) %>%
  filter(days_b_screening_arrest >= -30) %>%
  filter(is_recid != -1) %>%
...
Now we can measure the predictive accuracy and amount of bias in our model after applying sampling and compare to the bias before.

### Predictive Accuracy Before and After Sampling

Looking at our confusion matrix after sampling, we get:
...
Here we see our predictive accuracy is comparable to the accuracy before sampling.
...
(post_sample_risk_black_v_white <- sigmoid(model_intercept + black_coef) / sigmoid(model_intercept))
```

Taking the ratio and subtracting from 1, we compute the change in relative risk before and after:

```{r}
pre_sample_risk_black_v_white <- relative_risk_black_v_white
1 - post_sample_risk_black_v_white / pre_sample_risk_black_v_white
```

Unfortunately, it looks like sampling did not help with our level of bias. The level of bias is about the same before and after -- the slight difference we see here could simply be due to variation in the cross-validation process. Though our preliminary bias-mitigation technique was not successful in this instance, this fact still gives us insight into bias in the COMPAS data. There are still some important takeaways we can draw from our experimentation, discussed in the next section.

# Takeaways

In this section we discuss what we think are the major takeaways from the above analysis. Some of these observations here are based on direct results from above while others are based on our best hypotheses about bias in this data given the results we saw. 

One important takeaway from our use of sampling and the fact that the level of bias did not change before and after is that bias can permeate features other than protected attributes. We performed sampling for one variable -- the race variable -- but were unable to decrease racial bias. This could suggest that other features in the data served as proxies for race, so despite the fact that we created balance between recidivate and non-recidivate observations between blacks and whites, the presence racial bias across other features still drove the racial bias in our model. This occurrence is consistent with some techniques from the literature suggested for minimizing algorithmic bias, such as suppression, which we saw among Kamiran and Calders’ four suggested techniques. 

Another takeaway is that using only the most basic features about defendants (instead of a detailed survey of variables, as used by COMPAS), as compared to the COMPAS risk of recidivism scores, we were able to produce a model with higher accuracy and lower levels of bias (both estimated via cross-validation). Our analysis certainly did not provide a comprehensive comparison, and COMPAS scores may be superior in many ways to our risk scores; however, the difference in performance and bias that we saw, given how simple our model is, suggests that COMPAS scores could be significantly improved.

# Future Work

To continue this project, there are several areas upon which we would expand. In addition to generating risk of recidivism scores, we would also explore risk of violent recidivism and perform a similar comparison to COMPAS scores as above. We would also work to improve our risk of recidivism models in terms of feature generation and accuracy and try other modeling approaches in addition to logistic regression. There is also much work to be done in eliminating bias from these models. While our attempt via uniform sampling did not work, there are several other techniques for data preprocessing (such as those mentioned in the report) to eliminate bias that could be more fruitful. Furthermore, we did not explore any methods that impose fairness constraints on the models themselves. A hybrid approach that does both may be necessary to successfully minimize bias. It would also be interesting to do a more in depth exploration of the trade-off between bias and performance and see how this trade-off manifests in predictions from the COMPAS data.

# References

[1]  Angwin, Julia, et al. "Machine bias: There’s software used across the country to predict future criminals. and it’s biased against blacks." ProPublica, May 23 (2016).

[2] Kamiran, Faisal, and Toon Calders. "Data preprocessing techniques for classification without discrimination." Knowledge and Information Systems 33.1 (2012): 1-33.

[3] Larson, Jeff, et al. "How we analyzed the COMPAS recidivism algorithm." ProPublica (5 2016) (2016).

[4] https://github.com/propublica/compas-analysis 

[5] http://www-hsc.usc.edu/~eckel/biostat2/slides/lecture16.pdf 
